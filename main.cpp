...
char * tuple_key_to_string(tuple_t p)
{
	char * temp=(char *) malloc(100);
	memset(temp,0,100);
	memcpy(temp,&(p.key.src_ip),sizeof(p.key.src_ip));
	memcpy(temp+sizeof(p.key.src_ip),&(p.key.dst_ip),sizeof(p.key.dst_ip));
	return temp;
}
...
map< vector<int>, int> mlfreq;
map< vector<int>, int> are10;
...
int main(int argc, char** argv)
{
    ...
    char *filename="/usr/sketch/temp.pcap";
    adapter_t* adapter = pcap_init(filename);

    double memory = 0.025;//MB
    int word_size = 64;
    ...
    while(1)
    {
		...
		if (adapter_next(adapter, &p, &status) == -1) {
			break;
		}


		if (status == STATUS_VALID){
			char *ttmp=tuple_key_to_string(p);
			strcpy(insert[package_num],ttmp);
			free(ttmp);
		}
...
/********************************query*********************************/
    ...
    printf("**************prone*****************\n");
    FILE *fid1=fopen("prone.txt","w");
    cmsketch->get_prone_item(mlfreq,query,unmp.size(),10,unmp);
    map< vector<int>, int>::iterator itt1 = mlfreq.begin();
    for(;itt1!=mlfreq.end();itt1++)
    {
    	for(auto k:itt1->first)
    	{
    		fprintf(fid1,"%d ",k);
    	}
    	fprintf(fid1,"%d\r\n",itt1->second);
    ...
    FILE *fid2=fopen("prone_are_aae.txt","w");
    cmsketch->get_prone_item_are_aae(are10,query,unmp.size(),0.5,0,unmp);
    map< vector<int>, int>::iterator itt2 = are10.begin();
    for(;itt2!=are10.end();itt2++)
    {
    	for(auto k:itt2->first)
    	{
    		fprintf(fid2,"%d ",k);
    	}
    	fprintf(fid2,"%d\r\n",itt2->second);
    ...
    ofstream f("all_flow_with_aae_fangcha.txt");
    map< vector<int>,double> mmlfreq;
    cmsketch->get_all_item_with_aae_fangcha(mmlfreq,query,unmp.size(),unmp);
    map< vector<int>, double>::iterator itt3 = mmlfreq.begin();
    for(;itt3!=mmlfreq.end();itt3++)
    {
    	for(auto k:itt3->first)
    	{
    		f<<k<<" ";
    	}
    	f<<setprecision(3)<<itt3->second<<endl;
...
/********************************************************************************************/
    ...
    vector<int> print_all;
    FILE * fid_all=fopen("all_flow.txt","w");
    ...
    for(unordered_map<string, int>::iterator it = unmp.begin(); it != unmp.end(); it++)
    {
        ...
        print_all=cmsketch->print_item(temp,unmp);


        for(auto j:print_all)
        {
        	fprintf(fid_all,"%d ",j);
        	//printf("%d ",j);
        }
